...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['create_member'])) {
        ...
        // Get the next ID
        $result = $conn->query("SELECT MAX(id) AS max_id FROM gold_members");
        $row = $result->fetch_assoc();
        $next_id = $row['max_id'] + 1;

        $sql = "INSERT INTO gold_members (id, name, phone, password, due_date, workout_sessions, personal_trainer_phone)
                VALUES ($next_id, '$name', '$phone', '$password', CURDATE(), '$workout_sessions', '$personal_trainer_phone')";
        ...
        $personal_trainer_phone = $_POST['edit_personal_trainer_phone'];

        $sql = "UPDATE gold_members SET name='$name', phone='$phone', due_date='$due_date', workout_sessions='$workout_sessions', personal_trainer_phone='$personal_trainer_phone' WHERE id=$id";
        ...
        $sql = "DELETE FROM gold_members WHERE id=$id";
        ...
        $result = $conn->query("SELECT payment_status FROM gold_members WHERE id=$id");
        $row = $result->fetch_assoc();
        ...
        $sql = "UPDATE gold_members SET payment_status='$new_status' WHERE id=$id";
...
$members_sql = "SELECT id, name, phone, due_date, payment_status, workout_sessions, personal_trainer_phone FROM gold_members";
...
<html>
<head>
    <title>Gold Members Management</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <h2>Gold Members Management</h2>

        <div class="new-member-form">
            <h3>Create New Member</h3>
            <form action="gold_members.php" method="post">
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="checkbox-group">
                        ...
                        <label><input type="checkbox" name="workout_sessions[]" value="Steam Bath"> Steam Bath</label>
                    </div>
                </div>
                <div class="row">
                    <label for="personal_trainer_phone">Personal Trainer Phone:</label>
                    <input type="text" id="personal_trainer_phone" name="personal_trainer_phone" required maxlength="10" pattern="\d{10}">
                </div>
                ...
                if ($members_result->num_rows > 0) {
                    while($row = $members_result->fetch_assoc()) {
                        echo "<tr>
                                ...
                                <td>
                                    <form action='gold_members.php' method='post' style='display:inline;'>
                                    ...
                                    <a href='edit_gold_member.php?id={$row['id']}'>Edit</a>
                                </td>
                            </tr>";
                    }
                } else {
